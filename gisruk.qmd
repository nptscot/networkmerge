...
- The `parenx` Python for image skeletonisation and Voronoi diagram-centreline identification is available on PyPI in the GitHub repo [`anisotropi4/parenx`](https://github.com/anisotropi4/parenx).
...
The key contributions of the paper are the novel methods of image skeletonisation[@scikit2014; @zha1984; @lee1994], presented in @sec-simplification-via-skeletonization, and simplification with Voronoi diagrams to identify central lines, covered in @sec-simplification-via-voronoi-polygons.
...
The skeletonisation approach is based on the idea of creating a buffer around the network and then skeletonizing the buffer.
...
The skeletonisation approach is more computationally efficient, but the Voronoi approach yields more accurate (in terms of true centreline) and less 'wavy' results.
