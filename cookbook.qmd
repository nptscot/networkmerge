...
# Simplification
The `skeletonize.py` and `voronoi.py` simplication scripts will now simplify linear geometry. Example road and rail data for Leeds, Edinburgh and Doncaster used during development is available[^7]. Both allow the maximum displacement (simplify)[^8] and buffer size (buffer) to be set. 
...
The helper script sets and installs `parenx` in a virtual enviroment, and runs the `skeltonize` and `voronoi` simplification scripts with a number of different simplification parameters, and converts the output into a sanitized `GeoJSON` format if `ogr2ogr` is installed. It takes two option command line parameters that specify the path to data and output filename.
