...
Both input datasets were sourced from the Propensity to Cycle Tool (PCT) which in turn is based on OpenStreetMap (OSM) (see the '`parenx` cookbook', a technical appendix that accompanies this paper, for an example of the method applied to a rail network dataset).
...
The skeletonization approach generates a simplified network by buffering the network, applying an image skeletonization algorithm, and extracting lines segments from a raster of this buffer.
...
The skeletonization algorithm is then applied to the raster image yielding a skeletal raster image, as shown in @fig-thin-skeleton.
...
Noting that creating a line geometry from the set of points in the raster buffer is arguably the most complex step.
