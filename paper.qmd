...
The increasing complexity of route network data from sources like OpenStreetMap [@openstreetmap] allows for more detailed modelling of sustainable transport modes such as walking and cycling.
...
This paper addresses these issues by presenting and evaluating two distinct methods for simplifying complex route networks: 1) a raster-based approach using image skeletonization, and 2) a vector-based approach using Voronoi diagrams to identify centrelines.
...
The methods are implemented in the open-source `parenx` Python package, ensuring they are reproducible, adaptable, and ready for application in new transport planning contexts.
...
Transport network datasets are *spatial networks* composed of nodes (junctions) and edges (links or ways), where each edge has an associated cost, such as its length or travel time [@barthélemy2011].
These components are geographically located, with edges representing the physical infrastructure of the transport network, often enriched with attributes like the type of way (e.g., motorway, cycle path), its physical characteristics, and usage data such as daily traffic volumes.
Route network simplification can be applied to both input and output networks, aiming to reduce complexity while preserving the essential spatial structure and relevant attributes of the network.

The growing availability of high-resolution geographic datasets, from open sources like OpenStreetMap and authoritative mapping agencies, has enabled the creation of increasingly detailed maps.
...
Similarly, vector smoothing techniques can improve the aesthetic quality of network visualisations but do not simplify the underlying network structure [@pradhan2023modified; @duong2022statistical].
...
Our approach is based on techniques that can be applied to any set of overlapping linear features where aggregation is desired, such as simplifying transport or other flow networks to create a consolidated route-view for capacity analysis or strategic modelling.
...
To demonstrate the practical application and effectiveness of the proposed simplification techniques, we apply them to a complex urban road network in the city centre of Edinburgh, Scotland.
This area is characterised by a dense network of streets, including multi-lane roads, one-way systems, and historic streets, making it a challenging and realistic test case for network simplification.
The input dataset for this area, as shown in the top-left panel of @fig-results, contains numerous parallel lines representing dual carriageways and other complex road features.

We apply both methods with consistent parameters to enable a fair comparison.
The results of our simplification methods are illustrated in the remaining panels of @fig-results.
The skeletonized network (top right) provides a simplified representation of the input dataset.
This method effectively collapses the dual carriageways into single centrelines, while preserving the overall connectivity of the network.
The resulting network is visually much cleaner than the original, making it easier to interpret traffic flows or other network attributes that might be displayed on it.

The Voronoi-based simplification (bottom left) offers an alternative simplified network.
Like the skeletonization approach, it successfully merges parallel lines.
Visually, the lines produced by the Voronoi method can be smoother and more geometrically regular than those from the skeletonization method, which may be advantageous for certain cartographic applications.
<!-- TODO: Add a sentence here to compare the skeletonized and Voronoi results for Edinburgh in more detail, if possible. For example, are there any noticeable differences in how they handle specific intersections? -->

Finally, the primal network (bottom right) represents the highest level of simplification.
...
This level of abstraction is particularly useful for strategic network analysis, where the focus is on connectivity and network structure rather than precise geographical representation.
For example, it could be used as an input for graph-based analysis of network resilience or for creating schematic maps of the transport system.
...
This paper has presented and demonstrated two novel methods for simplifying complex route networks: one based on image skeletonization and the other on Voronoi diagrams.
...
The Voronoi approach, while more computationally intensive, can produce smoother and more aesthetically pleasing centrelines.
...
A key advantage of our implementation in the `parenx` Python package is its flexibility, allowing users to tailor the simplification process from minor adjustments to the aggressive simplification of a 'primal network', as shown in the [`parenx` cookbook](https://nptscot.github.io/networkmerge/cookbook.html).

There are several promising directions for future research building on this work.
...
The speed of the process could be improved through parallelisation, implementation in lower-level programming languages such as Rust, or further optimisation of the core algorithms.
...
We believe these tools have significant potential in other areas of transport planning, such as strategic rail network modelling, public transport network design, and the redesign of urban street networks to create more sustainable and equitable transport systems.
