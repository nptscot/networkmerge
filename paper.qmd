...
In this approach lines are buffered and combined to form a raster image and thinned to produce to a skeletal remnant that preserves the extent and connectivity centred on a line centred on the combined buffered region.

Using the Edinburgh GeoJSON network as above:


#### Create a projected combined buffered geometry:

::: {#fig-base-and-buffered-network layout-ncol="2"}
```{python}
...
base_nx.plot()
nx_geometry = get_geometry_buffer(base_nx["geometry"], radius=8.0)
nx_geometry.plot()
```
...
A scaled affine transformation is applied to the projected coordinate geometry to create a scaled raster image. The raster image is then cleaned to remove small holes in the image, typically where buffer lines run parallel or intersect at shallow angles. The image is then thinned and the resulting skeleton raster image transformed back to a point geometry but still in raster geometry.

This is an issue as the intent is to provide a simplified set of lines not points that lie on these lines.
...
transform_point.plot()
```

### Convert from a projected point to a line geometry
...
nx_output = nx_output.map(shapely_transform).map(set_precision_pointone)
nx_output.plot()
```
### Primal network
There are circumstances where it would be interesting to see a "primal" network, only consisting of line start and end points, as here,
