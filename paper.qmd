...
In this approach lines are buffered and combined to form a raster image and thinned to produce to a skeletal remnant that preserves the extent and connectivity centred on a line centred on the combined buffered region, using the Edinburgh GeoJSON network as above.


#### Create a projected combined buffered geometry:

An outer buffer, in this case `8.0m`, is applied to the base network.
...
The buffered street network to be simplified is shown in @fig-base-network
```{python}
...
#| - Coordinate (GB1936)
fig, ax = plt.subplots()
#base_nx.plot(edgecolor="blue", color="blue", ax=ax).grid()
nx_geometry.plot(edgecolor="black", color="blue", ax=ax).grid()
```
...
print(f"Rasterio affine transformation:\t{pd.Series(r_matrix)}")
print(f"Shapely affine transformat:\t{np.asarray(s_matrix).reshape(2, -1)}")
...
But the issue with this is that we need a simplified set of lines not a set of points that lie on the simplified network.
