...
As highlighted in the field of transport geography, network data models serve as digital representations of transportation networks, crucial for planning and operational purposes. These models, enriched through graph theory, encapsulate the complex, multi-modal nature of transportation data across various jurisdictions [^2]. The shapefile data model, developed by ESRI in the 1990s, has become a pivotal standard in storing vector information for network data models, facilitating the creation and analysis of network information [^2].

[^2]: See the [online documentation] (https://transportgeography.org/contents/methods/network-data-models/) 

In terms of topological representation, network data models necessitate a node and a link table for accurate topology construction. These tables are integral to defining network topology as structured by graph theory, although further developments are often required for applications in cartography and routing [^2]. This highlights the importance of an accurate and efficient topological representation in route network datasets, which can significantly impact their usability in transport planning.
...
Another approach, used in this paper, is to represent transport networks in standard geographic file formats, as a series of linestrings with attributes such as way type and flow.
Making transport datasets compliant with the 'simple features' geographic data specification in this way has advantages, <!-- compared with the proliferation of formats used by proprietary software, --> enabling easier sharing of datasets.[^2]

[^2]: The simple features standard is formalised by the International Organization for Standardization in [ISO 19125-1:2004](https://www.iso.org/standard/40114.html) and implemented in a wide range of file formats such as ESRIs shapefile, GeoJSON, and the open standard for geographic data, GeoPackage.
...
Sustainable transport planning can benefit from the influx of data, but excess complexity and intricacy of street network geometries can lead to problems.
The problem of network simplification is not new.
It can be seen as a component of wider 'map generalization' methods for pre-processing datasets depending on the scale of analysis [@sutton1998].
