...
<!--- WD: I'm not sure I understand this
...
This can lead to incorrect inferred traffic flow values, misinterpretation of model outputs and, potentially, investment in the wrong places.
...
Transport network datasets are *spatial networks* "for which the nodes are located in a space equipped with a metric", typically a two-dimensional Euclidean (or weighted Euclidean) distance metric [@barthélemy2011].
...
Transport networks must contain spatial information on the coordinates of nodes (points along ways) and the edges that connect them (straight lines between nodes representing ways).
<!--- WD: What is a segment in this context
They often contain attributes such as the type of way (e.g. road, rail, cycle), its physical characteristics (e.g. is lit at night), and usage data, e.g. daily traffic using each segment.
As inputs, route networks typically represent road or other transport networks along which vehicles travel.
WD: we use "metric" to mean a geometric distance metric, or simply as a measurement. This could be confusing
...
--->
They often contain attributes such as the type of way (e.g. road, rail, cycle), its physical characteristics (e.g. is lit at night), and usage data, e.g. daily traffic for each way.
As input, route networks represent road or other transport networks along which vehicles travel.
When output, they typically visualize parameters such as flow over different time periods on transport networks at the segment level [@sutton1998].
...
--->
With the increasing availability of high resolution geographic datasets and performant hardware and software, people (e.g. via OpenStreetMap) and national mapping agencies are mapping in increasing detail.
Sustainable transport planning benefits from this influx of data, but the complexity and intricacy of street network geometries may lead to problems in analyzing and visualizing transportation networks.
...
While preserving the overall shape and geographical accuracy, this method struggles to reduce network complexity, as shown in @sec-methods.
Vector smoothing approaches, including the use of Bezier curves [@pradhan2023modified] and Kernel-based smoothing [@duong2022statistical], can create more visually appealing lines or polygons, but do not reduce the number of vertices and so are of limited use for simplifying transport networks.
...
Recent implementations that both import and export linestrings include the under-development `neatnet` Python package [@fleischmann2024], and the `parenx` package [@deakin2024], the latter used in this paper.
...
See the ['`parenex` cookbook'](https://nptscot.github.io/networkmerge/cookbook.html) and [Methods](https://nptscot.github.io/networkmerge/methods.html) appendicees for further details on the methods used in this paper and their application to alternative (railway based) datasets.
...
Where knots are short segments at intersections, resembling tangled knots.
...
There are at least a couple of implementations of network joining approaches in open source software: the `rnet_merge()` function in the `stplanr` R package [@lovelace2019], and the [`rnetmatch` R
<!--- WD: I'm not sure what this means
