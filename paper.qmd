...
The image below illustrating the process of creating a skeletal remnant from a street network has been generated. It depicts the transition from buffered street lines to a raster image, followed by a thinning process that results in a skeletal remnant, while retaining the original network's extent and connectivity.

![Illustration of Skeletonization](pics\illustrating the process of creating a skeletal remnant from a street network.png)
...
- `rnet_x` - Target Route Network: This dataset represents our primary framework. It is a streamlined, simplified version of a more intricate network, serving as the foundational geometry. rnet_x is akin to a canvas, prepared to receive additional layers of data.

- `rnet_y` - Source of Additional Attributes: In contrast to rnet_x, rnet_y brings depth and detail. It is the repository of rich, comprehensive attributes that are missing from our target network. 

## Step 1: Coordinate Reference System Alignment

### Objective
Uniformity in spatial referencing is crucial for accurate data analysis. 
...
### Objective
Customize the attribute integration process based on specific requirements.
...
### Objective
Integrate attributes from `rnet_y` into `rnet_x` effectively.
...
- `segment_length` (Segment Maximum Length): This specifies the upper limit for each segment's length within the network, here set to 10 meters. Segmenting the network in this manner enhances manageability and contributes to more precise localization of lines within the specified buffer. This granularity is key for achieving higher accuracy in attribute integration.
...
![image](https://github.com/nptscot/networkmerge/assets/74598734/9c191046-8ea5-4ed9-8430-7ffd6fe809d0)
...
Once we obtained  the rnet_merged which is asimplied network but alos contians deitlas attrivbutes. We then need a series of steps to refine the data.
