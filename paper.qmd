...
otley_edge_sk = otley_edge.map(shapely_transform).map(set_precision_pointone)
otley_edge_sk.plot()
```

```{python}
armley_edge = get_nx(armley_line)
armley_edge_sk = armley_edge.map(shapely_transform).map(set_precision_pointone)
armley_edge_sk.plot()
```
...
r_matrix_edinburgh, s_matrix_edinburgh, out_shape_edinburgh = get_affine_transform(edinburgh_geometry, scale=2.0)
shapely_transform = partial(affine_transform, matrix=s_matrix_edinburgh)
edinburgh_skeleton = get_skeleton(edinburgh_geometry, r_matrix_edinburgh, out_shape_edinburgh)
...
edinburgh_edge_sk = edinburgh_edge.map(shapely_transform).map(set_precision_pointone)
...
r_matrix_doncaster, s_matrix_doncaster, out_shape_doncaster = get_affine_transform(doncaster_geometry, scale=2.0)
shapely_transform = partial(affine_transform, matrix=s_matrix_doncaster)
doncaster_skeleton = get_skeleton(doncaster_geometry, r_matrix_doncaster, out_shape_doncaster)
...
doncaster_edge_sk = doncaster_edge.map(shapely_transform).map(set_precision_pointone)
```

```{python}
edinburgh_boundary = get_geometry_line(edinburgh_geometry)
edinburgh_voronoi = get_voronoi(edinburgh_boundary, tolerance, scale)
edinburgh_line = get_voronoi_line(edinburgh_voronoi, edinburgh_boundary, edinburgh_geometry, radius)
...
doncaster_line = get_voronoi_line(doncaster_voronoi, doncaster_boundary, doncaster_geometry, radius)
