...
Another approach adopted in this paper involves the use of shapefile data (.shp) to represent transportation networks. The shapefile data model, developed by ESRI in the 1990s, has become a pivotal standard in storing vector information for network data models, facilitating the creation and analysis of network information. This format effectively represent transport networks in standard geographic file formats, as a series of linestrings with attributes such as way type and flow.
Making transport datasets compliant with the 'simple features' geographic data specification in this way has advantages, <!-- compared with the proliferation of formats used by proprietary software, --> enabling easier sharing of datasets.[^3]

[^3]: The simple features standard is formalised by the International Organization for Standardization in [ISO 19125-1:2004](https://www.iso.org/standard/40114.html) and implemented in a wide range of file formats such as ESRIs shapefile, GeoJSON, and the open standard for geographic data, GeoPackage.
    For ease of data sharing, we share transport networks used in this paper as plain text GeoJSON files.

With the increasing availability of high resolution geographic datasets and performant hardware and software, citizens (e.g. via OpenStreetMap) and national mapping agencies are mapping more and more detail.
...
Progress towards solving this knotty problem has included new methods for the automatic detection of 'face artifacts' [@fleischmann] and removal of 'slivers' to generate simplified representations of 'street blocks' [@grippa2018]. 

In Geographic Information Systems (GIS), vector simplification and smoothing are essential techniques for refining and optimizing vector data representations, yet they are not without their limitations. Vector simplification, which includes methods like the Douglas-Peucker algorithm [@liu2020vector] and the Visvalingam-Whyatt algorithm [@de2014efficient], is focused on reducing the number of vertices in a line or polygon feature. While effective in preserving the overall shape and geographical accuracy, this method can struggle with reducing the complexity of the route layout. It might oversimplify intricate network structures, potentially leading to a loss of critical information or misinterpretation of network connectivity.

Vector smoothing, encompassing techniques such as Chaikin’s algorithm, Bezier curves [@pradhan2023modified], and Kernel-based smoothing [@duong2022statistical], aims to create more visually appealing lines or polygons by mitigating sharp angles and irregularities. However, this approach can sometimes result in the distortion of spatial relationships within the network. It might smooth out important details that are critical for accurate analysis, especially in highly detailed or complex network structures. Furthermore, both vector simplification and smoothing, in their pursuit of creating more manageable and aesthetically pleasing representations, can inadvertently overlook the functional aspects of transportation networks, such as traffic flow patterns and junction intricacies.



As highlighted in previous research, network simplification has many real world applications.
...
In this study, we focus on two exemplary input datasets, as outlined in @tbl-input-data, both of which are sourced from the Propensity to Cycle Tool (PCT).
The PCT derives its data from OpenStreetMap (OSM), each illustrating varying levels of complexity.
