...
However, this complexity often complicates the visualization of transport model results, leading to potential misinterpretation in strategic network planning and investment decisions.
A common issue is the presence of multiple parallel ways on the same corridor, which can lead to visual artefacts such as incorrect flow values inferred from the corridor, potentially leading to the misinterpretation of results and investment in the wrong places.
To address this challenges, we present and compare two methods for *simplifying* route network datasets: image skeletonization and Voronoi diagram-based approaches.
...
Being developed with open data and open-source software, these methods are not only reproducible but also adaptable for future extensions in various contexts.
...
The affine transformations for Rasterio and Shapely are demonstrated with a scaling factor of 2.0. The Rasterio transform applies a scale and translation in a specific order, while the Shapely transform follows a different order for scaling and rotation, as illustrated in Table @tbl-panel.
...
```{python}
otley_im = rif.rasterize(otley_geometry.values, transform=r_matrix_otley, out_shape=out_shape_otley)
with warnings.catch_warnings():
    warnings.simplefilter("ignore")
    otley_im = remove_small_holes(otley_im, 20).astype(np.uint8)

rip.show(otley_im, cmap="Blues", title="buffer geometry")
```

```{python}
armley_im = rif.rasterize(armley_geometry.values, transform=r_matrix_armley, out_shape=out_shape_otley)
with warnings.catch_warnings():
    warnings.simplefilter("ignore")
    armley_im = remove_small_holes(armley_im, 20).astype(np.uint8)
rip.show(armley_im, cmap="Blues", title="buffer geometry")
```
...
The image undergoes a thinning process, yielding a skeletal raster image as the result. This skeletonized image effectively captures the essential structure and layout of the original network, as illustrated in @fig-thin-skeleton.

::: {#fig-thin-skeleton layout-ncol="2"}

```{python}
otley_skeleton = skeletonize(otley_im).astype(np.uint8)
rip.show(otley_skeleton, cmap="Blues", title="skeleton geometry")
...
rip.show(armley_skeleton, cmap="Blues", title="skeleton geometry")
...
The challenge with this approach is that instead of generating points situated on the simplified network, it returns a set of points rather than line geometries. This necessitates the inference of line geometry from the associated set of points.
...
After adjacent points are identified, line segments are created by connecting these points. The final and crucial step is the amalgamation of these individual line segments. This combination results in a continuous line geometry that represents the simplified network.
<!-- This conversion from point to line geometry is a pivotal aspect of network simplification. -->
To visualize the simplified network in its original spatial context, the reverse affine transformation is applied. This step reverts the network back to its original coordinate system, aligning the simplified geometry with the original spatial framework.
...
```{python}
shapely_transform = partial(affine_transform, matrix=s_matrix_otley)
nx_output = otley_line.map(shapely_transform).map(set_precision_pointone)
nx_output.plot()
plt.show()
```
...
nx_output = armley_line.map(shapely_transform).map(set_precision_pointone)
nx_output.plot()
plt.show()
```
...
```{python}
otley_line = get_raster_line_without_knot(otley_line.to_frame("geometry"))
nx_output = otley_line.map(shapely_transform).map(set_precision_pointone)
nx_output.plot()
plt.show()
```
...
```{python}
armley_line = get_raster_line_without_knot(armley_line.to_frame("geometry"))
nx_output = armley_line.map(shapely_transform).map(set_precision_pointone)
nx_output.plot()
plt.show()
```

Simplified versions of the Otley Road (left) and Armley Road (right) networks, with knots removed.
...
nx_output = otley_edge.map(shapely_transform).map(set_precision_pointone)
nx_output.plot()
```

```{python}
armley_edge = get_nx(armley_line)
nx_output = armley_edge.map(shapely_transform).map(set_precision_pointone)
nx_output.plot()
```
...
otley_segment.plot(edgecolor="red", linestyle='--', linewidth=1)
plt.show()
```

```{python}
armley_segment = get_segment_nx(armley_boundary, scale).reset_index(drop=True)
armley_segment.plot(edgecolor="red", linestyle='--', linewidth=1)
plt.show()
```

Segmented boundaries of the Otley Road (left) and Armley Road (right) networks.

:::

As with the skeletonize approach, the the simplified LineString geometries are converted into point geometries.
...
The Voronoi diagram is generated from nx_point within the bounds of nx_envelope, as illustrated in the figures below.
...
Voronoi diagrams of the Otley Road (left) and Armley Road (right) networks.
...
When attribute$value corresponds to the sum function, it is crucial to ensure accuracy and relevance in the results. The complexity of the original network, characterized by multiple lines, implies a diverse distribution of attributes. A weighted sum method is essential here, as it ensures that each attribute from the parallel lines is accurately represented in the aggregate value for the single line in the simplified network. This approach accounts for the varying significance and characteristics of each line in the complex network, ensuring that more influential lines have a proportionate impact on the final aggregated value. The formula for the weighted sum is:
...
Normalization is the next crucial step. It adjusts the aggregated values from the complex network, making them suitable for representation in the simplified network. This process is vital in ensuring that the simplification does not distort the true values of attributes and relationships as they exist in the complex network. Normalization helps translate the data into a form that is consistent and meaningful within the context of the simplified representation. The normalization formula is:

```
Normalized Value = Weighted Sum / ∑ wi
```

The adjustment for overestimation, achieved through the over_estimate factor, becomes indispensable in this context. It serves as a corrective mechanism to align the summed and normalized values from the complex network, such as parallel lines, with the single line of the simplified network. This factor ensures that the attribute values are not only aggregated but also scaled appropriately to match the simplified network's representation. The formula for adjustment of overestimation is:

```
over_estimate = ∑(Aggregated Value × Length of Single Line) / ∑(Original Value × Length of Lines in Complex Network)
Adjusted Value = Normalized Value / over_estimate
```

In essence, when transferring attribute values from a complex network to a simplified single-line network, employing a strategic approach is crucial. This approach includes using weighted summation to accurately aggregate values from multiple sources, normalization to ensure these values are meaningful within the simplified network, and an adjustment for overestimation to ensure the final values are proportionate and representative of the original network's attributes. This meticulous process ensures that the simplified network accurately reflects the complexity and nuances of the original network.
...
To calculate max_angle_diff, it's necessary to accurately calculate the angles (bearings) of lines from two networks, rnet_x$angle_x and rnet_y$angle_y. The difference in these angles is then computed as rnetj$angle_diff <- rnetj$angle_y - rnetj$angle_x. In this study, the line_bearing function is defined to calculate the angle. As shown below,

```{r}
rnet$angle <- line_bearing(rnet_y, bidirectional = TRUE)
```
The process of calculating the bidirectional bearing of a line segment in geographical coordinates involves normalizing the standard bearing so that it is the same regardless of the direction of travel along the line. Below is the detailed formula:
...
3. **Convert Bearing from Radians to Degrees**:
   ```
   θ = θ × 180 / π
   ```

4. **Normalize Bidirectional Bearing to [0, 180) Degrees**:
   ```
   θ = θ mod 180
   ```
In this bidirectional adjustment, the bearing is normalized to a range of [0, 180) degrees. This means that opposite directions along a line yield the same bearing value. This approach is particularly useful in scenarios where the directionality of the line is not a primary concern, ensuring consistency in bearing values regardless of the line's travel direction.
