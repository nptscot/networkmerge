...
An example of the issue is shown in @fig-pct from the Propensity to Cycle Tool for England (PCT) @lovelace2017.
The left panel shows Otley Road with an estimated commuter cycling flow of 818 (@fig-otley-road).
The right panel, by contrast, shows three parallel ways parallel to Armley Road with a cycling potential of 515 (shown), 288 and 47 (values not shown) (@fig-armley-road).
...
Illustration of the impacts of rasterisation on the visualisation of parallel ways: the rasterisation process can lead to visual artefacts, such as the 'staircase' effect shown in the right hand figure.
Source: open access Propensity to Cycle Tool results available at www.pct.bike.
:::
...
tm_shape(input_simplified, bbox = tmaptools::bb(input_simplified, 1.1)) +
  tm_lines() +
  tm_facets(by = "simp_factor", free.coords = TRUE)
```
