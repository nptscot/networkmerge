...
Finally, the primal network (bottom right) is the most simplified version of the input dataset, with only the most essential connections retained. This highly simplified network is created by identifying all nodes in the network, counting the edges connected to each node, and removing all nodes that are connected to exactly two edges. Unconnected edges or those at intersections between edges are retained.
