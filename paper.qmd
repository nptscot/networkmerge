...
Both of which are sourced from the Propensity to Cycle Tool (PCT) which in turn is based on OpenStreetMap (OSM) (see the '`parenex` cookbook', a technical appendix that accomanies this paper, for an example of the method applied to a rail network dataset).
...
The raster image also requires pre-processing to eliminate small holes that appear where buffered lines run parallel or intersect at shallow angles. The result of the scale and cleaned raster is illustrated in @fig-rasterize.
```{python}
...
Line segments are then created by connecting these adjacent points. These points are combined, giving a continuous line geometry representing the simplified network.
...
Both skeletonization and Voronoi simplified networks require post-processing to remove unwanted [knots](@sec-knots).
