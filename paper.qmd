...
input_simplified1 = purrr::map_dfr(simplification_levels, .id = "id", function(x) {
  rmapshaper::ms_simplify(input_projected, keep_shapes = TRUE, keep = x)
})

input_simplified2 = lapply(
  simplification_levels,
  function(x) {
    res = rmapshaper::ms_simplify(input_projected, keep_shapes = TRUE, keep = x)
    res$id = x
    res
  }
  )
input_simplified2 = do.call(rbind, input_simplified2)
waldo::compare(input_simplified1, input_simplified2)
```
