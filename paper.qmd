...
Finally, the primal network (bottom right) is the most simplified version of the input dataset, with only the most essential connections retained. This highly simplified network is created by identifying all nodes within an edge, counting the edges at that node and removing all nodes with an edge with count of two. Retains unconnected edges, or those at intersections between edges.
