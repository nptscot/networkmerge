...
This approach was taken as we could only find few examples for the aggregation and simplification of linear network features, especially for transport and planning networks. Where this general approach is based on the application of techniques used for image simplification, and applied to any overlapping linear features where aggregation is appropriate. For example, simplification of transport or other flow networks to produce an aggregated route-view as the basis for capacity analysis. Whether that is for active-transport, rail, road or other logical network flows.
