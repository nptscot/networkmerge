...
Transport network datasets are *spatial networks* "for which the nodes are located in a space equipped with a metric", space typically being two-dimensional space and the main metric being Euclidean (or weighted Euclidean) distances [@barthélemy2011].
...
They often contain attributes such as the type of way (e.g. road, rail, cycle), its physical characteristics (e.g. is lit at night), and usage data, e.g. the amount of daily traffic using each segment.
...
Route network simplification can be applied to both inputs and outputs of transport models, with the aim of reducing complexity while preserving the spatial structure of the network and relevant attributes.
...
Recent implementations that both import and export linestrings include the under-development `neatnet` Python package [@fleischmann2024] , and the `parenx` package [@deakin2024], the latter of which is used in this paper.
...
Network simplification is a challenge that is not unique to transport planning.
...
In this paper we focus the two street networks discussed in the previous section, the extents and geometries of which are shown in @fig-input-data.
Both of which are sourced from the Propensity to Cycle Tool (PCT) which in turn is based on OpenStreetMap (OSM).
(See the '`parenex` cookbook', a technical appendix that accomanies this paper, for an example of the method applied to a rail network dataset, and the `methods` technical appendix for a detailed description of the methods used in this paper.)
...
In skeletonization, overlapping lines are identified, buffered, transformed into a raster image, the image processed through a thinning algorithm, and a skeletal representation of the original network produced (see the Methods appendix for details).
...
Voronoi lines that are completely enclosed within the buffer geometry and are situated at a distance of less than half the buffer's width from the buffer edge.
...
Simplified versions of the Otley Road (left) and Armley Road (right) networks, using skeletonization (top) and Voronoi (bottom) approaches.

:::

## Post-Processing

Both skeletonization and Voronoi simplified networks require post-processing to remove unwanted knots.
Optionally by removing intermediate lines-sections, a further simplified or primal network that captures the essential connectivity and layout of transport routes can be generated.

Knots are short segments at intersections, resembling tangled knots.
To remove these features of networks short segments are clustered together, and a central point for each cluster is determined.
...
The primal network is composed of direct lines connecting start and end points.
This primal network represents an extreme form of simplification, prioritising the network's structure and compression.
...
See Section 4 in the [Methods appendix](https://nptscot.github.io/networkmerge/methods.html) for details.
...
It is clear the the geometry of the input dataset (top left) is more complicated than needed for strategic transport planning.
...
Matrix of Route Network Simplification Stages across Edinburgh.
:::
...
This paper explored methods for simplifying route networks and demonstrated reproducible implementations.
The two main methods developed for this purpose were image skeletonization and Voronoi diagram-centreline identification.
Both resulted in geometries that were substantially simpler and less resource-intensive for subsequent network analysis steps.
Furthermore, a follow-on stage of joining the attributes of the original 'complex' network, aggregating values where needed, was shown to substantially improve visualisation and interpretation of the simplified network.

Both methods successfully reduced the complexity of the network representations while preserving spatial and connectivity characteristics.
The appropriate balance between simplification and preservation of network geometries will vary depending on the specific requirements of the analysis.
An advantage of the implementations we have developed, available in the Python package `parenx`, is that they are highly flexible and can be adapted to suit the specific requirements of the user, from low levels of simplification to aggressive simplification towards the 'primal network'.

There are several future directions of travel that we believe are worth exploring, building on the work presented here.
...
A limitation of the approach is that it is time-consuming for large networks such as cities, motivating research and software development to speed up the process, with options including parallelisation, implementation in lower-level languages, and optimisation of the algorithms themselves.
...
However, we believe it has applications beyond this use case, in strategic rail network modelling, network redesign work, and in the development of computationally efficient and easy-to-interpret and modify network representations for use in transport planning and analysis.
