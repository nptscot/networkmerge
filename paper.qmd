...
See the [Methods](https://nptscot.github.io/networkmerge/methods.html) and [Cookbook](https://nptscot.github.io/networkmerge/cookbook.html) appendices for details on the methods, implementation, and how to run the pipelines on your own data.
...
It is notable that the `neatnet` implementation results in a network that is similar to the Voronoi output, but with some differences such as effective removal of the complex roundabout at the top of the sub-figures and a single 'stranded' linestring in the central western part of the network.
...
ax2.set_title('Official Road Centrelines (Ordnance Survey OpenRoads)')
...
# Plot 4: Voronoi
ax4 = edinburgh_line.plot(ax=ax4, color='black')
...
```{r}
#| fig.cap: "Results of the route network simplification methods applied to Edinburgh city centre, comparing the input dataset (top left), ground truth network (top right), skeletonised network (middle left), Voronoi diagram (middle right), primal network (bottom left) and neatnet simplification (bottom right)."
#| out-width: 100%
#| label: fig-results
plot_files = c(
...
Recent performance benchmarks comparing the computational efficiency of `neatnet` and `parenx` implementations [@fleischmann2025adaptivecontinuitypreservingsimplificationstreet] suggest substantial room for improvement in our implementation, motivating future work on computational efficiency in both Voronoi-based and skeletonisation approaches.
