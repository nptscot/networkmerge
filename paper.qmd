...
```{python}
i, j = base_otley.sindex.query(combined_armley, predicate="intersects")
...
otley_geometry = pd.concat([p, q])
try:
    otley_geometry = gp.GeoSeries(list(unary_union(otley_geometry.values).geoms), crs=CRS)
except AttributeError:
    otley_geometry = gp.GeoSeries(unary_union(otley_geometry.values), crs=CRS)
otley_geometry.plot()
```
...
armley_geometry = pd.concat([p, q])
try:
    armley_geometry = gp.GeoSeries(list(unary_union(armley_geometry.values).geoms), crs=CRS)
except AttributeError:
    armley_geometry = gp.GeoSeries(unary_union(armley_geometry.values), crs=CRS)
armley_geometry.plot()
```
