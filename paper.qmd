...
Although the focus of this paper is converting linestrings to linestrings, a common way of achieving this is through the intermediate step of converting linestrings to polygons and then converting these polygons back into linestrings representing centrelines or *medial axes* (an approach used in both methods presented in @sec-methods), with numerous algorithms and implementations for medial axis calculation published in the literature and open source software repositories [e.g. @smogavec2012; @centerli2023].
Recent implementations that both import and export linestrings include the under-development `neatnet` Python package [@fleischmann2024] , and the `parenx` package [@deakin2024].
The latter is the basis of the methods presented in this paper.
