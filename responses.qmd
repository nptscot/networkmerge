...
- [x] **R2.3** Clarify the `parenx` API; address unusual CLI model and `skeletonize.py`/`voronoi.py` naming.
...
- The package now exposes the core simplfication functions in the `parenx` namespace, so users and developers can import them directly in their Python code. The following now works thanks to this change, which previously would generate an error:
